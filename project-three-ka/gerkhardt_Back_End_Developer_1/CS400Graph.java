...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
      if (!vertices.containsKey(start) || !vertices.containsKey(end)) {
        throw new NoSuchElementException("Does not contain either start or end value");
      }
      // Set the shortest path to null for initialize
       Path shortestPath = null;
       // Create lists to check and keep track of
       List<Path> pathList = new LinkedList<Path>();
       List<Vertex> vertexList = new LinkedList<Vertex>();
       List<Path> endList = new LinkedList<Path>();
       // Make the vertex list hold all the vertices of the graph, which is used for the while loop
       for (int i = 0; i < vertices.size(); i++) {
         vertexList.add(vertices.get(i));
       }
       // Create the priority queue, which will be used to access the shortest path
       PriorityQueue<Path> paths = new PriorityQueue<Path>();
       Vertex init = vertices.get(start);
       Path first = new Path(init);
       paths.add(first);
       // While loop to run through while there are unchecked vertices
       while (!vertexList.isEmpty()) {
         shortestPath = paths.poll();
         pathList.add(shortestPath);
         // Immediately can hop out if the shortest path ends on the ending vertex
         if (shortestPath.end.equals(end)) {
           return shortestPath;
         }
         // Keep track of all of the edges from this shortest path's end vertex
         List<Edge> edgesLeaving = shortestPath.end.edgesLeaving;
         // Remove this vertex from the list, as it has been visited already
         vertexList.remove(shortestPath.end);
         // This for loop checks through all of the edges coming off from the vertex
         // Then it checks if that vertex hasn't been visited, which it then adds
         // it to a list for later and puts it on the queue
         for (int i = 0; i < edgesLeaving.size(); i++) {
           if (vertexList.contains(edgesLeaving.get(i).target)) {
             Path currentPath = new Path(shortestPath);
             currentPath.extend(edgesLeaving.get(i));
             paths.add(currentPath);
           }
         }
         
       }
       // Filters out from the list of paths to only include those that end on the correct ending
       for (int i = 0; i < pathList.size(); i++) {
         if (pathList.get(i).end.equals(end)) {
           endList.add(pathList.get(i));
           
         }
       }
       // If there are no ends, everything is jumped through to return an error
       if (!endList.isEmpty()) {
         shortestPath = endList.get(0);
       }
       for (int i = 0; i < endList.size(); i++) {
         if (shortestPath.compareTo(endList.get(i)) > 0) {
           shortestPath = endList.get(i);
         }
       ...
       // Throw a no such element exception if none were found after searching through
