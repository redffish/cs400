...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path source) {
      ...
      this.end = source.end;
      this.dataSequence = new LinkedList<>();
      for (T item : source.dataSequence) {
        this.dataSequence.add(item);
      }
    ...
    public void extend(Edge edge) {
      dataSequence.add((T) edge.target.data);
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // immediately checks if the start or end is null
    if (start == null || end == null || vertices.get(start) == null || vertices.get(end) == null) {

      throw new NoSuchElementException("Start or End node was null");
    }
    ...
    paths.add(shortestPath); //add start to the queue
    visited.add(shortestPath.start); // add start to the list visited


    while ((!(paths.isEmpty()))) {
      pathToExplore = new Path(paths.poll()); // polls the shortest path in the queue
      if (pathToExplore.end.data.equals(end)) { // breaks loop if what is passed (which is the
                                                // shortest current path) already contains the end
        break;
      }

      // loops through to add every possible path
      for (int i = 0; i < pathToExplore.end.edgesLeaving.size(); i++) {
        Path pathToAdd = new Path(pathToExplore); // create path that can be modified
        pathToAdd.extend(pathToExplore.end.edgesLeaving.get(i)); // extends the path

        if (visited.contains(pathToAdd.end)) { // if the newly made path contains something already
                                               // visited, don't add it and continue thru loop
          continue;
        }

        paths.add(pathToAdd); // adds the modified path back to the queue
        visited.add(pathToAdd.end); // adds the newly added vertex to visited
      }
    }

    if (!(pathToExplore.end.data.equals(end))) { // if the end result after loop ends does not
                                                 // contain end, throw exception
      throw new NoSuchElementException("Path not found");
    }
    return pathToExplore; // else return the path
  }
