...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			// Implemented this constructor in Step 5.
			this.start = source.start; // set starting node
			this.distance = source.distance; // copy distance
			this.dataSequence = new LinkedList<T>(source.dataSequence); // alternate constructor to copy in a list
			this.end = source.end; // set end node
		}
		...
		public void extend(Edge edge) {
			// Implemented this method in Step 6.
			this.end = edge.target; // update end
			this.distance += edge.weight; // add weight
			this.dataSequence.add(edge.target.data); // add to list
		}
	...
	protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
		if (start == null || end == null || vertices.get(start) == null) {
			throw new NoSuchElementException("The start or end vertex were null.");
		}
		PriorityQueue<Path> queue = new PriorityQueue<Path>(); // will hold the current paths
		Path toReturn = new Path(vertices.get(start)); // the shortest path that has the proper start and end nodes that
														// will
														// be returned
		LinkedList<Vertex> visitedNodes = new LinkedList<>(); // keeps track of the nodes that have been visited
		boolean containsEnd = false; // is the end in the path?
		Vertex tempVertex = vertices.get(start);
		Path tempPath = new Path(tempVertex);
		queue.add(tempPath); 
		visitedNodes.add(tempVertex); // add starting node to visited list
		while (!queue.isEmpty()) {
			// get the next path
			tempPath = queue.poll();
			tempVertex = tempPath.end; // vertex is now the end node from the current path
			if (tempVertex.data.equals(end)) { // if we met end
				containsEnd = true; // update value
				break; // found shortest, get out
			}
			for (int i = 0; i < tempVertex.edgesLeaving.size(); i++) {
				if (visitedNodes.contains(tempVertex.edgesLeaving.get(i).target)) {
					// node is already visited
					continue;
				} else {
					// node isn't visited
					Path copy = new Path(tempPath);
					copy.extend(tempVertex.edgesLeaving.get(i)); // extend the path
					queue.add(copy);
					visitedNodes.add(tempVertex.edgesLeaving.get(i).target); // record node as visited
					
				}
			}
		}
		// once we have found all the paths, find if there is a valid path to the end node
		if (containsEnd) {
			toReturn = tempPath; // good to return
			return toReturn;
		} else {
			throw new NoSuchElementException("Search failed to find any path from the start to end vertices.");
		}
	}
