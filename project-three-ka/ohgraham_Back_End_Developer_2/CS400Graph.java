...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        PriorityQueue<Path> frontier = new PriorityQueue<>(vertices.size()); // Auto-sorted, shortest paths first.
        Hashtable<T, Path> found = new Hashtable<>();
        // Find the start and end vertices.
        Vertex startVertex = vertices.get(start);
        if (startVertex == null) throw new NoSuchElementException("No vertex containing start can be found");
        if (!vertices.containsKey(end)) throw new NoSuchElementException("No vertex containing end can be found");
        for (Path path = new Path(startVertex); path != null; path = frontier.poll()) {
            // If this path isn't the shortest one to its end vertex found so far, don't use it.
            Path foundPath = found.get(path.end.data);
            if (foundPath != null && foundPath.distance <= path.distance) continue;
            // Save this path as the shortest path to its end node.
            found.put(path.end.data, path);
            // Prepare to look at the paths extending from thie path's end.
            for (Edge edge : path.end.edgesLeaving) {
                Path futurePath = new Path(path);
                futurePath.extend(edge);
                frontier.add(futurePath);
            }
        }
        // Grab the path to the specified vertex and return it.
        Path shortest = found.get(end);
        if (shortest == null) throw new NoSuchElementException("No path from start to end can be found");
        return shortest;
    }
