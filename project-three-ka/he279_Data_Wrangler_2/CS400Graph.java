...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
          ...
          this.dataSequence = new LinkedList<>();
          Iterator<T> iter = source.dataSequence.iterator();
          while (iter.hasNext()) 
        ...
        public void extend(Edge edge) {
          ...
          this.distance += getWeight(this.end.data, edge.target.data);
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
      ...
      if (this.containsVertex(start) == false || this.containsVertex(end) == false) {// if start or end vertex does not exist
        throw new NoSuchElementException("Cannot take vertices that do not exist as parameters");
      }
      // create a pq to hold the discovered paths(total distance + vertex data) 
      // that have not yet determined to be the shortest possible paths to their given end vertex
      PriorityQueue<Path> tempShortest = new PriorityQueue<Path>();
      Hashtable<T, Path> paths = new Hashtable<>();  // store all possible paths
      List<T> visited = new LinkedList<>(); // keep track of whether vertex is visited
      
      for (Vertex v: vertices.values()) {
        paths.put(v.data, new Path(v));  // add all paths to Hashtable
        if (v.data != start) {
          paths.get(v.data).distance = Integer.MAX_VALUE; // initialize all distances (except start) to infinity
        }
      }
      tempShortest.add(paths.get(start)); // add neighbors of start to pq
      
      while (!tempShortest.isEmpty()) {  // find shortest path among the neighbors
        Path shortest = tempShortest.poll();
        visited.add(shortest.end.data);
        if (shortest.end.data.equals(end)) 
          return shortest;
        else {
          for (Edge e: shortest.end.edgesLeaving) {  // make a path contains the new distance and compare with shortest
            Path newShortest = new Path(shortest);
            if (!visited.contains(e.target.data)) {
              newShortest.extend(e);
              tempShortest.add(newShortest);
            }
          }
        }
      }
