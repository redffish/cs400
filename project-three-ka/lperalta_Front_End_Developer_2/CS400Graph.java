...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {        
		if(!containsVertex(start) || !containsVertex(end)) {
			throw new NoSuchElementException("Given vertices not found.");
		}
		...
		while(!pq.isEmpty()) {
			Path c = pq.poll();
			if(c.end.data.equals(end)) {
				return c;
			}
			while(visited.contains(c.end.data)) {
				c = pq.poll();
			}
			visited.add(c.end);
			for(int i=0; i<c.end.edgesLeaving.size(); i++) {
				Path copy = new Path(c);
				copy.extend(c.end.edgesLeaving.get(i));
				pq.add(copy);
			}
		}
