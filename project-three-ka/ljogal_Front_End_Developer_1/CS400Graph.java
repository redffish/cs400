...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			// Set distance equal and intialize data sequence
			this.dataSequence = new LinkedList<>();
			this.distance = source.distance;
			T add = source.dataSequence.get(0);
			this.start = new Vertex(add);
			// Copy values from old edges leaving linked list into new
			for (int j = 0; j < source.start.edgesLeaving.size(); j++) {
				this.start.edgesLeaving.add(source.start.edgesLeaving.get(j));

			}
			// Copy all data into data sequence
			for (int i = 0; i < source.dataSequence.size(); i++) {
				this.dataSequence.add(source.dataSequence.get(i));
			}
			// Copy contents of end Vertex
			T addEnd = source.dataSequence.get(source.dataSequence.size() - 1);
			this.end = new Vertex(addEnd);
			for (int j = 0; j < source.end.edgesLeaving.size(); j++) {
				this.end.edgesLeaving.add(source.end.edgesLeaving.get(j));
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		// hold the discovered paths (including total cost distance, and previous vertex
		PriorityQueue<Path> paths = new PriorityQueue<>();
		// Put the nodes visited in here
		Hashtable<T, Vertex> visited = new Hashtable<>();		
		if(start == null || vertices.get(start) == null || end == null || vertices.get(end) == null) {
			throw new NoSuchElementException("No path found");
		}
		// Keep adding paths to the priority queue and pop them off as going along
		// back track after through visited nodes to find shortest path through nodes
		Vertex current = vertices.get(start); // current vertex being looked at, current is always updated 
		paths.offer(new Path(current)); // path is offered
		Path update = null;
		
		// want to end when end is found
		while(!paths.isEmpty()) {
			
			// Poll the best path and save into  a path
			update = paths.poll();
			
			if(update.end.data.equals(end)) {

				return update;
			}
			// Want to look at the end of the path
			current = update.end;		
					
		
			
			// Check potential edges from path at top of priority queue
			LinkedList<Edge> potEdgs = current.edgesLeaving;
			// Add potential paths to the priority queue, priority queue will keep shortest path in priority
			for(int i = 0; i < potEdgs.size(); i++) {
				Edge comp = potEdgs.get(i);

				Path add = new Path(update);
				add.extend(comp);
				// Keep going through paths until v isn't found, dont want to look at already visited nodes
				if(visited.contains(add.end.data)) {
					continue;
				}	
				paths.offer(add);
				
			}
			visited.put(current.data, current); // current put into visited hashtable	
		}
		// If gone through and nothing found then through expception
		throw new NoSuchElementException("No path found");
	}
